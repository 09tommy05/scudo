    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-8 py-6 border-b border-gray-100">
          <h1 class="text-2xl font-bold text-gray-800">{{ isNew ? 'Nuovo articolo' : 'Modifica articolo' }}</h1>
          <p class="text-sm text-gray-500 mt-1">{{ isNew ? 'Compila i campi per creare un articolo.' : 'Aggiorna i campi e salva.' }}</p>
        </div>
        ...
        <form @submit.prevent="save" class="p-8 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-gray-700 text-sm font-semibold mb-2">Titolo articolo *</label>
              <input
                v-model="form.title"
                type="text"
                required
                ...
                placeholder="es. Phishing via Email"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-start">
              <div class="w-full sm:w-64 aspect-video rounded-lg border border-gray-200 bg-gray-50 flex items-center justify-center overflow-hidden shrink-0">
                <img
                  v-if="imagePreview"
                  :src="imagePreview"
                  :alt="form.title"
                ...
                />
                <span v-else class="text-gray-400 text-sm">Nessuna immagine</span>
              </div>
              <div class="flex flex-col gap-2">
                <p class="text-xs text-gray-500">Attuale o nuova immagine. Carica un file per sostituirla.</p>
                <input
                  type="file"
                  accept="image/*"
                  @change="onImageChange"
                  class="block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-primary file:text-white file:font-semibold file:cursor-pointer"
                />
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            <input
              id="edit-article-draft"
              v-model="form.isDraft"
              type="checkbox"
              class="rounded border-gray-300 text-primary focus:ring-primary"
            />
            <label for="edit-article-draft" class="text-sm font-medium text-gray-700">Salva come bozza</label>
          </div>
          ...
          <div class="flex flex-wrap justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            >
              {{ saving ? 'Salvataggio...' : (isNew ? 'Crea articolo' : 'Salva modifiche') }}
            </button>
          </div>
        </form>
      </div>
...
const imagePreview = computed(() => {
  if (form.img instanceof File) {
    return URL.createObjectURL(form.img);
  }
  if (form.currentImg) return getImageUrl(form.currentImg);
  return null;
});
...
async function loadArticle() {
  ...
  try {
    const res = await api.getArticle(id.value);
    const a = res.data;
    form.title = a.title || '';
    form.short_text = a.short_text || '';
    form.categoria = a.categoria || '';
    form.currentImg = a.img || '';
    form.isDraft = !!a.isDraft;
    form.img = null;
    const html = a.text != null ? String(a.text) : '';
    form.text = html;
    ...
    error.value = err.response?.data?.message || err.message || 'Impossibile caricare l\'articolo';
  } finally {
    loading.value = false;
  }
}
...
async function save() {
  ...
  try {
    ...
    if (form.img instanceof File) payload.img = form.img;

    if (isNew.value) {
      await api.createArticle(payload);
    } else {
      await api.updateArticle(id.value, payload);
    }
