<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
      ...
      <div v-if="!mode" class="flex justify-center space-x-4 mb-6">
        <button @click="loginMode = 'operator'"
...
<script setup>
import { ref, onMounted, computed, defineProps, watch } from 'vue'; // Aggiunto import onMounted
...
const loginMode = ref(props.mode || 'operator');
...
const pageTitle = computed(() => {
  if (loginMode.value === 'operator') return 'Accesso Operatore';
  if (loginMode.value === 'user') return 'Accesso SPID / CIE';
  return 'Accedi a SCUDO';
});
